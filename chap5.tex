...
There is a defined data type \verb=objc_class_holder= which, however, is again only a redefined \verb=void*=. Using the run-times function pointers \verb=class_holder.creator=, \verb=class_holder.destroyer=, \verb=class_holder.lookup= and TODO, the run-time creates the data structure required, has means to destroy it, lookup a class according to its name and TODO.

The run-time provides a default implementation of this structure, a simple hash table which resembles slightly NXMapTable from Apple's run-time, but removes all dependencies and is made slightly less complicated.
